...
def resume_from_checkpoint(fdir, model, optimizer=None, scheduler=None):
    ...
    # Try to load with strict=False to handle size mismatches gracefully
    try:
        model.load_state_dict(checkpoint["state_dict"], strict=True)
        print("Loaded model weights")
    except RuntimeError as e:
        # If there's a size mismatch, load compatible weights only
        print(f"Warning: Size mismatch detected. Loading compatible weights only.")
        print(f"Error: {e}")
        ...
        for k, v in checkpoint_dict.items():
            if k in model_dict:
                if model_dict[k].size() == v.size():
                    ...
                    skipped_layers.append(f"{k} (size mismatch: {v.size()} -> {model_dict[k].size()})")
            else:
                skipped_layers.append(f"{k} (not in model)")
        
        model.load_state_dict(model_dict, strict=False)
        print(f"Loaded {len(matched_layers)} compatible layers")
        if skipped_layers:
            print(f"Skipped {len(skipped_layers)} incompatible layers")
            if len(skipped_layers) <= 10:  # Only print if not too many
                for layer in skipped_layers:
                    print(f"  - {layer}")
            print("Note: Starting training from epoch 0 due to incompatible layers")
            # Return 0 to start from scratch when there are incompatible layers
            return 0
