    ...
    def load_model(self, directory, epoch=None):
        ...
        for name in names:
            ...
            if not osp.exists(model_path):
                # If best model doesn't exist, try to load the last epoch model
                if model_file == "model-best.pth.tar":
                    # Find the latest checkpoint
                    checkpoint_file = osp.join(directory, name, "checkpoint")
                    if osp.exists(checkpoint_file):
                        with open(checkpoint_file, "r") as f:
                            last_checkpoint = f.readline().strip()
                        model_path = osp.join(directory, name, last_checkpoint)
                        print(f"Best model not found, using last checkpoint: {last_checkpoint}")
                    else:
                        raise FileNotFoundError('Model not found at "{}" and no checkpoint file exists'.format(osp.join(directory, name, model_file)))
                else:
